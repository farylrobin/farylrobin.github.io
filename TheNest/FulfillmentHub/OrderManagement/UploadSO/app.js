// v2.0 – Account / Season / SO uploader
...
form.addEventListener('submit', async e => {
  ...
  if (!soFile && !devCatalogFile && !amazonCatalogFile){
  ...
  if (devCatalogFile)    fd.append('devCatalog',        devCatalogFile);
  if (amazonCatalogFile) fd.append('amazonDevCatalog',  amazonCatalogFile);
  ...
  try {
    ...
    if (res.ok){
      showResult(`Workflow has started [${new Date().toLocaleString()}]`);
    } else {
      const txt = await res.text();
      showResult(`Failed To Send: ${res.status} ${res.statusText} – ${txt}`);
    }
  } catch (err){
    console.error(err);
